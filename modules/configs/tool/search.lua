return function()
	...
	-- When using fzf-lua for file search, make the default action open the
	-- selected file in a right-side vertical split (vsplit).
	local function ensure_vsplit_default(opts)
		if not use_fzf then
			return opts
		end
		opts = opts or {}
		opts.actions = opts.actions or {}
		-- fzf-lua passes the selected entry as a table; first element is the path
	...
	local function file_searcher(fzf_fn, tb_fn, git_only)
		return function()
			...
			-- Ensure default action for fzf file pickers is vsplit
			opts = ensure_vsplit_default(opts)
